...
const BasketContext = React.createContext(
    {
      alts:[], //alts=[] ==> alts du panier
      add: () => {}, // fn d'ajout d'un alt
      rm: () => {},//fn de retrait d'un alt
      update: () => {}, //fn de mise à jour des donnees glucidiques de l'alt
      empty: () => {}, //fn qui vide l'array alts, et met le compteur du panier à 0 ==> appelee dans Basket ==> fn emptyPlate(), du btn "Vider l'assiette"
      keyAlt:0, 
      //--
      ratio:"", //fn qui contient le ratio de l'user
      setRatio: () => {}, //fn de maj et de recuperation du ratio de l'user
      //--
      stickyHeader:() => {}, //fn qui gere le stick du menu dans le Header ==> pour MyHeader
      //--
      resizeHeader:() => {}, //fn qui gere le resize du menu dans le Header (cf MyHeader), quand l'user resize la page
      //--
      board:"", //recupere la liste des alts trouves par Searcher, ou un string qui dit qu'on n'a pas trouve d'alts
      getDataFromForm:() => {}, //fn qui recupere les alts trouves par Searcher 
      //--compteur d'alts : ici alts api et alts user confondus
      compteur:0, //contient la valeur affichée dans le bouton Panier
      setCompteur:() => {},
      formatNumber:() => {},
    }
  );
...
class AltsProvider extends React.Component {
  ...
  stickyHeader(fn){ //envoyer stickyHeader() en props à Header pour lui renvoyer une fn en parametre qui lie le header au scroll du windows, et provoque le sticky du header (MyHeader dans Header)
    window.onscroll = ()=> {
      fn();  
    };
  }

  resizeHeader(fn){ //maj de myHeader quand resize par le user, de la fenêtre
    window.onresize = ()=> {
      fn();  
    };
  }  
  ...
  getDataFromForm(searcherData){
    this.setState({board:searcherData});
  } 
  ...
  setCompteur(n){ //gere les soustractions et additions ==> prend 1 ou -1
    this.setState({compteur:this.state.compteur+=n});
  }

  formatNumber(string){ //pour s'assurer que les chiffres des glucides n'aient pas plus de 2 chiffres après la virgule ==> dans Alt, AltInBasket et Basket
